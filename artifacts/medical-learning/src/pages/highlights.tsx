...
import { Trash2, BookOpen, MapPin, Highlighter } from "lucide-react";
...
export default function Highlights() {
  ...
  const getBookTitle = (bookId: number) => {
    return books?.find(b => b.id === bookId)?.title || `Kitob #${bookId}`;
  };
    ...
    <Layout>
      <div className="p-8 h-full flex flex-col">
        <header className="mb-8">
          <h1 className="text-3xl font-display font-bold text-foreground">Belgilanganlar</h1>
          <p className="text-muted-foreground mt-1">Kutubxonadagi muhim belgilangan matlar</p>
        </header>
        ...
        {isLoading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center pb-20">
            ...
            <h3 className="text-2xl font-display font-bold text-foreground">Belgilanganlar topilmadi</h3>
            <p className="text-muted-foreground mt-2 max-w-md">Kitob oching va muhim qismlarni belgilash uchun matnni tanlang.</p>
          </div>
        ) : (
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                    ...
                    {highlight.pageNumber && (
                      <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-amber-500" /> {highlight.pageNumber}-sahifa</span>
                    )}
